...
All code provided assumes you are working from a directory `RDM_workshop`
### 1. Organization
...
Important to note, the way you manage your data model and ETL (Extract, Transform, Load) pipelines will influence the tools you use and whether JSON is relevant. That said, many concepts will be cross applicable. For the purposes of this tutorial we will go over JSON because we can quickly prototype data validation.
...
```
We can see the following errors by the interpreter
```
{
    "$schema":"./schema.json",
...
```
As expected, when we flip the values the schema catches the values are mismatched. What about an undeclared variable? We'll use the example `honorific`.
```
{
    "$schema":"./schema.json",
    "firstName" : 1, Incorrect type. Expected "string"
...
Navigating to `/7_external_ontology_validation/4_2_incorporating_external_ontology_validation/work`, we can view contents by performing:
```
ls
```
...
If we perform this for all our examples:
```
jq -r '.icd10' A.json \
  | xargs -I{} curl -s "https://icd10api.com/?code={}&desc=short&r=json" \
  | jq -e '.Response == "True"' \
